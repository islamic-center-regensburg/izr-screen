...
export const FetchIqamaTimes = async () => {
  let iqamaTimes = null;
  await axios.get(izr_server + "/iqamah").then((response) => {
    iqamaTimes = response.data[0];
  });
...
export function getNextPrayerTime(prayerTimes: DayPrayerTimes) {
  ...
  for (const prayer of prayerOrder) {
    if (prayerTimes.hasOwnProperty(prayer)) {
      const [prayerHour, prayerMinute] = prayerTimes[
        prayer as keyof DayPrayerTimes
      ]
        .split(":")
        .map(Number);
      if (
